...
const validateCred = (array) => {
    ...
    for (let x = 0; x < emptyArray.length; x++) {
        if (emptyArray[x] > 9) {
            finArray.push((emptyArray[x]) - 9)
        } else {
            finArray.push(emptyArray[x])
        }

    }
    let sum = finArray.reduce(function (a, b) {
        return a + b
    }, 0)
    mud = sum + array[array.length - 1]
    if (mud % 10 === 0) {
        return true
    } else {
        return false
...
console.log(findInvalidCards([invalid3]));
